...
const { userService, voucherService } = require('../services');
...
const transferCard = catchAsync(async (req, res) => {
  ...
  const card = await voucherService.transferCard(req.user, req.body.cardId, receiverObj);
...
const spendFromCard = catchAsync(async (req, res) => {
  ...
  const card = await voucherService.spendFromCard(
    req.user,
    req.body.cardId,
    req.body.amount,
    req.body.currency
...
const getBalance = catchAsync(async (req, res) => {
  ...
  const balance = await voucherService.getBalance(req.user, req.params.cardId, req.query.verbosity);
  res.status(httpStatus.OK).send(balance);
});

const getHistory = catchAsync(async (req, res) => {
  logger.info(`req user: ${req.user.userUuid} req query: ${req.query}`);
  const cardHistory = await voucherService.getHistory(req.user, req.params.cardId, req.query.verbosity);
