...
const createCard = async (adminObj, receiverObj, amount, currency, expirationDate, enforcementDate) => {
  logger.info(`appAdmin objdump: ${JSON.stringify(adminObj, null, 2)}`);
...
/**
 * Recharge card with provided amount of points by admin
 * @param {User} adminObj
 * @param {BigInteger} amount
 * @param {String} currency
 * @param {BigInteger} extendPeriodDays
 * @param {String} cardId
 * @returns {Object}
 */
const rechargeCard = async (adminObj, amount, currency, extendPeriodDays, cardId) => {
  ...
  try {
    logger.info(
      `admin rechargeCard submitAsync cardId: ${cardId} recharge amount: ${amount} prolong expiration by: ${extendPeriodDays} days`
    );
    const result = await chaincodeConnection.submitAsync('AdminRecharge', {
      arguments: [`${amount}`, `${extendPeriodDays}`, cardId],
    });
    logger.info(`admin rechargeCard result: ${JSON.stringify(JSON.parse(utf8Decoder.decode(result.getResult())))}`);
    const json = JSON.parse(utf8Decoder.decode(result.getResult()));
    json.result = assetUtils.reformatDate(json.result);
    return json;
  } catch (e) {
